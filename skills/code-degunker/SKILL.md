...
**Signals:** `var` instead of `let`/`const`, `componentWillMount` in React, `request` library (deprecated), `file moment.js` when `date-fns` or `Temporal` is available, `http.createServer` for complex apps instead of a framework, Python 2 syntax
