...
Save the API key as a secret in [Settings > Advanced](/?t=settings&s=advanced):
- **Secret name**: `TAVILY_API_KEY`
- **Value**: Your Tavily API key (starts with `tvly-`)

Then scripts will automatically pick it up via `os.getenv("TAVILY_API_KEY")`.

**Status**: ⚠️ Not yet configured — no TAVILY_API_KEY found in /home/workspace/credentials/.env. Get a key at https://tavily.com and save it as a Zo secret.
