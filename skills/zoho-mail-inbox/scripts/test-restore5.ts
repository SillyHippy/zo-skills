...
async function test() {
  ...
  // Use the working messages/view endpoint
  let allMessages: any[] = [];
  let start = 1;
  while (true) {
    const resp = await fetch(`${BASE_URL}/api/accounts/${ACCOUNT}/messages/view?folderId=${TRASH}&limit=200&start=${start}&includeto=false`, { headers: authHeaders });
  ...
  if (allMessages.length === 0) { console.log("Trash is empty - nothing to restore"); return; }
