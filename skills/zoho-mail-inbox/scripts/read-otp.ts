...
// Search for Zoho OTP email
const resp = await fetch(`${BASE}/api/accounts/${ACCOUNT}/messages/view?limit=3&includeto=false`, { headers: h });
const data = await resp.json();
console.log("Messages:", data.data?.length || 0);

for (const m of (data.data || []).slice(0, 3)) {
  console.log(`[${m.folderId}] ${m.subject?.substring(0, 80)}`);
  ...
  if (otpMatch) { console.log("OTP:", otpMatch[1]); console.log(otpMatch[1]); }
}
